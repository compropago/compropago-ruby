...
When an order changes its status, our platform will notify each of the registered routes with the order information modified in JSON format.
You must recover this JSON in a text string to later convert it to an object of type CpOrderInfo using the Factory class provided by the SDK in the following way:
info = Factory.get_instance_of 'CpOrderInfo', string_obtained
Generate the approval logic corresponding to the status of the order.
